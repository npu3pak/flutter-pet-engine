...
class PunctualLightBuffer {
  ...
  // The last uploaded contents and the texture they were uploaded to, so a
  // frame whose lights (and spot-shadow stamps) are unchanged reuses that
  // texture without re-uploading. The ring rotates a different slot into
  // use on every acquire, so a skipped frame must return the last uploaded
  // texture itself — acquiring a fresh slot would hand the shader an empty
  // or stale one (the flicker bug).
  Float32List? _lastParams;
  List<int>? _lastIndices;
  gpu.Texture? _lastParamsTexture;
  gpu.Texture? _lastIndicesTexture;
  ...
  ///
  /// Uploads are skipped when the packed contents are bitwise identical to the
  /// previous call: the texture the previous call uploaded to is returned
  /// again, which is the common case for a static scene. The CPU pack and BVH
  /// cull still run, but the host -> device upload bandwidth is saved every
  /// frame.
  PunctualLighting build({
    ...
    if (count == 0) {
      _lastParams = null;
      _lastIndices = null;
      _lastParamsTexture = null;
      _lastIndicesTexture = null;
    ...
    final paramsUnchanged = _listsEqualFloats(_lastParams, packed.params);
    final gpu.Texture paramsTexture;
    if (paramsUnchanged && _lastParamsTexture != null) {
      paramsTexture = _lastParamsTexture!;
    } else {
      paramsTexture = _paramsRing.acquire(_texelsPerLight, count);
      paramsTexture.overwrite(packed.params.buffer.asByteData());
      _lastParamsTexture = paramsTexture;
    }
    _lastParams = packed.params;
    ...
    final indicesUnchanged = _listsEqualInts(_lastIndices, cull.indices);
    final gpu.Texture indexTexture;
    if (indicesUnchanged && _lastIndicesTexture != null) {
      indexTexture = _lastIndicesTexture!;
    } else {
      indexTexture = _indexRing.acquire(indexWidth, indexHeight);
      ...
      _lastIndicesTexture = indexTexture;
    }
    _lastIndices = cull.indices;
  ...
  static bool _listsEqualFloats(Float32List? a, Float32List b) {
