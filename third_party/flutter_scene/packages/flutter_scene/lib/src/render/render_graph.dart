...
class TransientTexturePool {
  ...
  /// Rings retired by [clear] whose textures must stay referenced until the
  /// frames that may still use them have completed. Dropping the Dart
  /// references immediately lets the finalizer free the native image while
  /// the GPU still reads it as a render target, which crashes the Vulkan
  /// driver at render-pass begin (Mali) — GLES happens to defer deletion and
  /// survives. Kept for [framesInFlight] frames, the same window the pool
  /// already trusts for texture reuse.
  final List<_RetiredRings> _retired = [];

  /// Advances to the next frame's ring slot. Call once per frame before
  /// any [acquire] calls.
  void beginFrame() {
    _frame = (_frame + 1) % framesInFlight;
    for (var i = _retired.length - 1; i >= 0; i--) {
      final batch = _retired[i];
      if (--batch.framesLeft <= 0) _retired.removeAt(i);
    }
  }
  ...
  /// textures aren't kept alive — the old ones are released after
  /// [framesInFlight] frames, not immediately (see [_retired]).
  void clear() {
    if (_rings.isEmpty) return;
    _retired.add(_RetiredRings(Map.of(_rings), framesInFlight));
    _rings.clear();
  }
}

/// One retired set of transient rings, kept alive for [framesLeft] frames.
class _RetiredRings {
  _RetiredRings(this.rings, this.framesLeft);

  final Map<TransientTextureDescriptor, List<gpu.Texture?>> rings;
  int framesLeft;
}
...
/// A pass that writes precomputed entries into the blackboard before the
/// rest of the graph runs.
///
/// Used when a view reuses results another view of the same frame already
/// produced (the shared shadow atlas, prepass depth, or ambient occlusion):
/// the producing pass is omitted from this view's graph, and this pass makes
/// its published handles available to the consumers that follow. Runs first,
/// so it must be the first pass added to the graph.
class BlackboardSeedPass extends RenderGraphPass {
  BlackboardSeedPass(this.entries);

  final Map<Object, Object?> entries;

  @override
  String get name => 'BlackboardSeedPass';

  @override
  void execute(RenderGraphContext context) {
    entries.forEach(context.blackboard.set);
  }
}
...
class RenderGraph {
  ...
  /// Enables per-pass timing logs when the app is built with `perf.log`.
  static const bool _perfLog = bool.fromEnvironment('perf.log');
  static final Map<String, double> _passMs = {};
  static int _passWindowMs = 0;
  static int _passFrames = 0;
  ...
  void execute({
    ...
    for (final pass in _passes) {
      ...
      sw.stop();
      _passMs[pass.name] =
          (_passMs[pass.name] ?? 0) + sw.elapsedMicroseconds / 1000;
    }
    if (_perfLog) _flushPassLog();
  }

  static void _flushPassLog() {
    _passFrames++;
    final now = DateTime.now().millisecondsSinceEpoch;
    if (_passWindowMs == 0) _passWindowMs = now;
    if (now - _passWindowMs < 2000) return;
    final frames = _passFrames;
    final parts = _passMs.entries
        .map((e) => '${e.key}=${(e.value / frames).toStringAsFixed(2)}')
        .join(' ');
    debugPrint('[perf-pass] frames=$frames $parts');
    _passMs.clear();
    _passFrames = 0;
    _passWindowMs = now;
  }
