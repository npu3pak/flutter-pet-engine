...
/// Expands disconnected line [segments] (a flat list of endpoint pairs) into
/// a camera-facing triangle list with a constant screen-pixel width.
///
/// Pure: it takes the view-projection matrix, the camera position and the
/// viewport size, so it can run without a GPU context. Returns six vertices
/// (two triangles) per segment, ready for `MeshData.build` and an updatable
/// `MeshGeometry`; the vertex count always matches the segment count, so a
/// caller can refresh the buffers in place every frame. Segments fully
/// behind the camera collapse into zero-area quads; a segment crossing the
/// near plane is clipped to it.
({Float32List positions, Float32List normals}) expandLineSegments(
  ...
  final height = viewportSize.height;
  if (width <= 0 || height <= 0) {
    return (positions: Float32List(0), normals: Float32List(0));
  }
  final segmentCount = segments.length ~/ 2;
  final positions = Float32List(segmentCount * 6 * 3);
  final normals = Float32List(segmentCount * 6 * 3);
  final inverse = Matrix4.inverted(viewProjection);
  final half = widthPx / 2.0;
  var v = 0;
  void write(Vector3 position, Vector3 normal) {
    final i = v * 3;
  ...
  for (var i = 0; i + 1 < segments.length; i += 2) {
    ...
    var ca = viewProjection.transformed(Vector4(a.x, a.y, a.z, 1.0));
    var cb = viewProjection.transformed(Vector4(b.x, b.y, b.z, 1.0));
    if (ca.w <= _kNearClipW && cb.w <= _kNearClipW) {
      // Fully behind the camera: a zero-area quad keeps the vertex count
      // stable for in-place updates.
      for (var k = 0; k < 6; k++) {
        write(a, Vector3(0.0, 0.0, 1.0));
      }
      continue;
    }
    var pa = a;
    var pb = b;
    if (ca.w <= _kNearClipW) {
      final t = (_kNearClipW - ca.w) / (cb.w - ca.w);
      pa = a + (b - a) * t;
      ca = viewProjection.transformed(Vector4(pa.x, pa.y, pa.z, 1.0));
    } else if (cb.w <= _kNearClipW) {
      final t = (_kNearClipW - cb.w) / (ca.w - cb.w);
      pb = b + (a - b) * t;
      cb = viewProjection.transformed(Vector4(pb.x, pb.y, pb.z, 1.0));
    }
    // Screen-space direction of the segment, in pixels (y down).
    var screenX = (_ndcX(cb) - _ndcX(ca)) * width;
    var screenY = -(_ndcY(cb) - _ndcY(ca)) * height;
    var length = math.sqrt(screenX * screenX + screenY * screenY);
    if (length < 1e-9) {
      screenX = 1.0;
      screenY = 0.0;
    ...
    screenY /= length;
    // Perpendicular offset by half the pixel width, expressed in NDC.
    final offsetX = -screenY * half * 2.0 / width;
    final offsetY = -screenX * half * 2.0 / height;
    final aLeft = _unproject(
      inverse,
      ca.x + offsetX * ca.w,
      ca.y + offsetY * ca.w,
      ca.z,
    ...
    final normalA = _towardCamera(cameraPosition, pa);
    final normalB = _towardCamera(cameraPosition, pb);
    // Two triangles per segment, in the same winding as the polyline strip.
    write(aLeft, normalA);
    write(aRight, normalA);
    write(bLeft, normalB);
    write(aRight, normalA);
    write(bRight, normalB);
    write(bLeft, normalB);
  }
  return (positions: positions, normals: normals);
}

/// Clip-space `w` below which a point counts as behind the camera when
/// [expandLineSegments] clips a crossing segment.
const double _kNearClipW = 1e-4;
