...
class PhysicallyBasedMaterial extends Material {
  ...
  /// Per-material distance-fog start override in world units (see the
  /// `material_fog` FragInfo slot). `0` (the default) uses the scene fog's
  /// start; a positive value makes this material dissolve into the haze from
  /// that distance instead — decor billboards fade with the ground they stand
  /// on, no later than the scene ramp.
  double fogStartOverride = 0;
  ...
  void bind(
    ...
    //   [24]      float fog_start_override (0 = scene fog start)
