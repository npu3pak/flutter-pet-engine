...
class EngineLightingUniforms {
  ...
  /// Packs the `FogInfo` block (7 vec4s / 28 floats, see `shaders/fog.glsl`)
  ...
  static void bindFog(
    ...
    if (fog != null && fog.enabled && fog.mode != FogMode.none) {
      ...
      // params3: minOpacity (base haze at the start distance).
      buffer[12] = fog.minOpacity.clamp(0.0, 1.0);
      // color.
      buffer[16] = fog.color.x;
      buffer[17] = fog.color.y;
      buffer[18] = fog.color.z;
      ...
      if (light != null && fog.sunInScatter > 0.0) {
        buffer[20] = light.color.x * light.intensity;
        buffer[21] = light.color.y * light.intensity;
        buffer[22] = light.color.z * light.intensity;
        buffer[23] = 1.0; // has-sun
        final direction = lighting.directionalLightDirection ?? light.direction;
        buffer[24] = direction.x;
        buffer[25] = direction.y;
        buffer[26] = direction.z;
      }
    }
