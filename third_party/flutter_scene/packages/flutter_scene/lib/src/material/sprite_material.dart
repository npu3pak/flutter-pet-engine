...
/// Set [opaque] to route the sprite through the opaque pass with an alpha
/// test instead — the depth buffer then resolves ordering exactly (used by
/// the grass field).
///
...
class SpriteMaterial extends Material {
  ...
  /// When true the sprite is drawn in the opaque pass with an alpha test
  /// (discards alpha < 0.5, writes depth): ordering is resolved by the depth
  /// buffer instead of translucent back-to-front sorting.
  bool _opaque = false;
  bool get opaque => _opaque;
  set opaque(bool value) {
    _opaque = value;
    doubleSided = value; // opaque billboards are drawn double-sided
  }
  ...
  void bind(
    ...
    fragInfo[7] = _opaque ? 1.0 : 0.0; // alpha_test (opaque sprites)
    ...
    // The sprite shader carries the FogInfo block (fog.glsl) too.
