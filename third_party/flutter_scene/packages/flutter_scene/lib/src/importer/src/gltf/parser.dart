...
GltfMaterial _parseMaterial(Map<String, Object?> j) {
  ...
  GltfPbrSpecularGlossiness? pbrSG;
  if (extensions?['KHR_materials_pbrSpecularGlossiness'] is Map) {
    final sg = extensions!['KHR_materials_pbrSpecularGlossiness']
        as Map<String, Object?>;
    pbrSG = GltfPbrSpecularGlossiness(
      diffuseFactor: sg['diffuseFactor'] is List
          ? (sg['diffuseFactor'] as List)
                .cast<num>()
                .map((e) => e.toDouble())
                .toList(growable: false)
          : const [1.0, 1.0, 1.0, 1.0],
      diffuseTexture: _parseTextureInfo(sg['diffuseTexture']),
      specularFactor: sg['specularFactor'] is List
          ? (sg['specularFactor'] as List)
                .cast<num>()
                .map((e) => e.toDouble())
                .toList(growable: false)
          : const [1.0, 1.0, 1.0],
      glossinessFactor: ((sg['glossinessFactor'] as num?) ?? 1.0).toDouble(),
      specularGlossinessTexture: _parseTextureInfo(
