...
class _ViewSurface {
  ...
  /// Swapchain rings retired by a resize. The old textures stay referenced
  /// for [Surface._maxFramesInFlight] frames because command buffers from
  /// earlier frames may still render into or sample them; dropping the Dart
  /// references immediately lets the finalizer free the native image under
  /// the GPU, which crashes the Vulkan driver at render-pass begin (Mali).
  final List<_RetiredSwapchain> _retiredSwapchains = [];

  gpu.Texture nextSwapchainColor(Size size) {
    pool.beginFrame();
    for (var i = _retiredSwapchains.length - 1; i >= 0; i--) {
      final batch = _retiredSwapchains[i];
      if (--batch.framesLeft <= 0) _retiredSwapchains.removeAt(i);
    }
    if (size != _previousSize) {
      _cursor = 0;
      if (_swapchainColors.isNotEmpty) {
        _retiredSwapchains.add(_RetiredSwapchain(
          List<gpu.Texture>.of(_swapchainColors),
          Surface._maxFramesInFlight,
        ));
      }
...
/// One retired swapchain ring, kept alive for [framesLeft] frames.
class _RetiredSwapchain {
  _RetiredSwapchain(this.textures, this.framesLeft);
