...
Material buildMaterial(GltfMaterial? gm, List<Texture2D> textures) {
  ...
  if (gm.unlit) {
    ...
    m.name = gm.name ?? '';
    final pbr =
        gm.pbrMetallicRoughness ?? _fromSpecGloss(gm.pbrSpecularGlossiness);
    if (pbr != null) {
...
/// Converts a spec-gloss material into the engine's metallic-roughness
/// model: diffuse becomes the base color, metals are absent (metallic 0)
/// and glossiness inverts into roughness.
GltfPbrMetallicRoughness? _fromSpecGloss(GltfPbrSpecularGlossiness? sg) {
  if (sg == null) return null;
  return GltfPbrMetallicRoughness(
    baseColorFactor: sg.diffuseFactor,
    baseColorTexture: sg.diffuseTexture,
    metallicFactor: 0.0,
    roughnessFactor: (1.0 - sg.glossinessFactor).clamp(0.0, 1.0),
  );
}
