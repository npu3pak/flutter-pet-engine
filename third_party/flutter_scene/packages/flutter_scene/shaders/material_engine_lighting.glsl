...
uniform FragInfo {
  ...
  vec4 camera_forward;
  // Per-material distance-fog override (reuses an unused once-diffuse-SH
  // slot). x: fog start distance in world units; 0 = use the scene fog's
  // start, so far decor dissolves into the haze sooner than the ground
  // (matching the scene at the horizon). yzw unused.
  vec4 material_fog;
  vec4 diffuse_sh5;
  vec4 diffuse_sh6;
  vec4 diffuse_sh7;
