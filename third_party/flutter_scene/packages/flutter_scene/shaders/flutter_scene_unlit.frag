uniform FragInfo {
  ...
  float fade;
  // Alpha handling: 0 opaque, 1 mask, 2 blend.
  float alpha_mode;
  // Alpha-test threshold used when alpha_mode is 1 (mask).
  float alpha_cutoff;
}
frag_info;
...
void main() {
  ...
  // MASK alpha mode: discard fragments below the cutoff, render the rest
  // fully opaque (glTF treats MASK output as binary). Mirrors the standard
  // material's coverage so the depth/mask passes agree.
