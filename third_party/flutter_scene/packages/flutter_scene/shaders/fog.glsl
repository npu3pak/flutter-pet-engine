...
uniform FogInfo {
  ...
  vec4 params2;
  // x: minimum opacity (base haze at the start distance)   yzw: unused
  vec4 params3;
...
// fog color by the distance-based fog factor, with a per-material fog start:
// when [start_override] > 0 the fog ramps from that distance instead of the
// scene fog's start (decor that should dissolve into the haze earlier than the
// ground it stands on). [sky_color] is the environment radiance sampled in the
// view direction (the lit path supplies it; the unlit path passes the flat fog
// color, making the sky mix inert); the fog color lerps from the flat color
// toward it by the sky-color influence, so far geometry dissolves into the sky
// behind it. Returns the fogged premultiplied color; alpha (coverage) is left
// unchanged so a transparent fragment adds no fog (the geometry behind it is
// fogged by its own, farther fragment).
vec4 ApplyFogAt(vec4 premult_color, vec3 sky_color, float start_override) {
  ...
  if (start_override > 0.0) {
    start = min(start, start_override);
  }
  ...
  // Blend from the base haze ([params3.x] at the start distance) up to the
  // far cap ([params0.z]) as the raw ramp grows 0 -> 1.
  fog_factor = mix(fog.params3.x, 1.0, fog_factor);
...
// [ApplyFogAt] with the scene fog's own start (no per-material override).
vec4 ApplyFog(vec4 premult_color, vec3 sky_color) {
  return ApplyFogAt(premult_color, sky_color, 0.0);
}
