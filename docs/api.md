...
- состояние жеста живёт в экземпляре обработчика ввода: пока указатели
  удерживаются, вьюпорт продолжает отдавать события тому экземпляру,
  который получил их `onPointerDown`, даже если виджет пересобрали с новым
  `SceneInput`; подмена применяется после отпускания последнего указателя.
  Стабильный экземпляр (поле состояния) всё равно предпочтителен — при
  создании в `build` каждый кадр плодятся объекты;
...
- `FlyCameraController.configureForExtent(extent)` — near/far/скорость
  полёта для крупных карт: до 200 единиц диагонали — дефолты, дальше
  скорость растёт пропорционально
  (`defaultFlySpeed · extent / flySpeedReferenceExtent`, где 94 — диагональ
