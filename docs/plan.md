...
>
> Моя схема очевидно слишком наивна, но у нас есть проекты, которые
> позволят подобрать такую же простую и понятную, но не наивную схему:
> `mypet-game`, `pet_engine/engine/example`.
>
...
- Старый движок и старые проекты сейчас не переписываем. Сначала делаем
  ...
  каталоге v2. Проектируем так, чтобы покрыть нужды всех проектов.
  Когда закончим — отдельно пересаживаем на v2 `mypet-game`.
...
- Старые репозитории (`pet_engine`, `mypet-game`) в этой работе не
  переписываются. Допустимы только инфраструктурные правки, связанные с
  переездом форка (пути зависимостей и замки пакетов).
...
Сначала составлен полный реестр возможностей проектов
...
- Перед миграцией игр — визуальные прогоны по сценам.
...
- **Документ и undo**: `AppState` на `SceneController(mergeStatic: false)`,
  ...
  шаблоны дома/комнаты, обработка изображений.
...
### Хвосты при пересоздании вьюпорта (13 сентября 2026)

Владелец: после переключения моделей могут остаться прошлые границы (снимки
«Работал с комнатой» / «Переключился на кресло»). Причина —
`EditorScene.dispose` снимал с контроллера только frame-listener: при
пересоздании `EditorViewport` (вкладка «Ресурсы» → назад; `MainScreen`
строит вьюпорт заново) `overlays` (сетка/рамка/курсор/выделение),
`metaOverlay.root`, `lightGizmoRoot` и оба гизмо оставались на общем
`SceneController`. Старый комплект замирал на прошлой модели — вокруг
кресла 3×3 оставались рамка и сетка комнаты 5×5, гизмо дублировались.

Исправлено:

- `EditorScene.dispose` снимает все свои ноды и гизмо с контроллера;
  `MetaOverlayLayer.dispose` освобождает кэш текстур пузырей и защищён от
  поздней асинхронной растеризации;
- движок: `ModelRenderer.clear()` — выгрузка документа чистит ноды и
  glTF-рантаймы (`unloadModel`/`rebuild` при `model == null`);
- `AppState.openProject` выгружает прошлый документ после успешного
  открытия нового проекта (проект без моделей больше не оставляет старую
  сцену).

Визуальная проверка: харнесс `pet.perf.drag` тапает «Ресурсы» →
«Композиция», меняет модель и снимает вьюпорт; до фикса — рамка/сетка
комнаты поверх кресла, после — чисто (`temp/perf_screenshots/remount_*`).
Регрессии: `editor_viewport_test` («пересоздание вьюпорта снимает старые
оверлеи и гизмо»), `app_state_test` («openProject без моделей выгружает
прошлый документ»), `picking_test` (unloadModel чистит документ).
...
### Техдолг перф-прохода: поворот, оверлеи, коалесинг (13 сентября 2026)

Закрыты три пункта из «Осталось на будущее» `docs/perf_journal.md`:

- **Поворот «запечённых» видов без пересборки.** `ModelRenderer.
  updateObjectTransforms` получил `rotated: {id: ΔR}` — модельный шаг
  `R_new·R_old⁻¹`; движок сам строит мировой префикс `T(anchor)·Δ·T(−anchor)`
  и сопрягает зеркало `M·Δ·M` для запечённых скруглений/CSG; редактор считает
  фактический снапнутый шаг, `beginRotateDrag` раскрывает выборку через
  `moveExpansion`. Поворот вставки/glTF: 53 мс → 0.10 мс, 59 полных
  пересборок → 0. Скрытый операнд CSG по-прежнему идёт полным rebuild.
- **Инкрементальные оверлеи.** Сетка/рамка/курсор кэшируются (пересоздание
  только при смене модели/размера), контур выделения — персистентная нода
  (перенос сдвигает трансформ, поворот пересобирает только контур),
  билборд-контур обновляется по смене yaw; три дублирующих пути пересборки
  сведены к `syncDocument`/`syncUiState`; драги меты/света обновляют только
  свой слой. Драг вставки: 0.44 → 0.13 мс.
- **Коалесинг pointer-событий.** Позиция гизмо-драга копится и применяется
  раз в кадр (`queueGizmoDrag`/`flushGizmoDrag`), на отпускании и отмене —
...
#### Камера крупных карт: скорость полёта (14 сентября 2026)

- **Проблема.** После конвертации карт WAD 1:1 WASD-полёт в редакторе был
  почти незаметен: `configureForExtent` ускорял камеру лишь как
  `extent / 300`, тогда как E1M1 (~4577×2817) требует примерно ×45 к
  легаси-темпу.
- **Правка.** Скорость стала пропорциональной диагонали с калибровкой от
  легаси-сетки: `defaultFlySpeed · extent / 94` (для диагоналей ≤ 200 —
  ровно 12 ед/с, как раньше); одиночный шаг нажатия WASD теперь тоже идёт
  от текущей скорости (`GameCamera.keyDown` принимает `speed`).
- **Проверки.** `engine` — analyze чист, 601 тест (обновлён
  `camera_extent_test.dart`: формула, линейность, шаг нажатия);
  `scene_editor` — analyze чист, 386 тестов; golden-эталоны совместимости
  и `fly_defaults` не сдвинулись.

#### Сетка земли: 1 px и скрытие (14 сентября 2026)

- **Правка.** Линия сетки земли в редакторе стала 1 px (`GridNode.lineWidthPx`
  вместо 1.5); в нижней панели рядом с «Wireframe всей сцены» появилась
  кнопка «Сетка земли», скрывающая оверлей (`AppState.gridVisible` →
  `EditorScene.setGridVisible`; при пересборке сетки видимость сохраняется).
- **Диплинк.** `settings?grid=0|1` — для визуальных прогонов; пример в
  `scene_editor/README.md`.
- **Проверки.** `scene_editor` — analyze чист, 386 тестов (добавлены
