uniform FrameInfo {
  ...
  vec4 params;
  // x: screen-parallel yaw (radians) for `kScreenParallel` facing.
  vec4 extra;
}
frame_info;
...
// Stage outputs. The standard engine varyings come first, in the exact order
// material_vertex.glsl declares them, so a billboard drawn into the depth
// prepass or the selection-mask pass (whose fragment shaders include
// material_varyings.glsl) assigns matching interpolant locations. The
// sprite-specific varyings follow, in the order flutter_scene_sprite.frag
// declares them.
out vec3 v_position; // world-space position
out vec3 v_normal; // camera-facing normal (unused by the depth/mask passes)
out vec3 v_viewvector; // camera_position - vertex_position (world space)
out vec2 v_texture_coords; // flipbook-aware sprite UV
out vec4 v_color;
// Sprite-specific varyings: the flipbook cell UVs (v_uv primary, v_uv2 the
// crossfade neighbour) and blend factor, plus the planar view-space depth for
...
const float kScreenParallel = 3.0;
...
void main() {
  ...
  } else if (facing == kScreenParallel) {
    // Upright, uniformly screen-parallel (all quads share one yaw, exactly
    // like the decor billboards driven by `screenParallelYaw`/`reorientBillboard`).
    float yaw = frame_info.extra.x;
  ...
  v_viewvector = frame_info.camera_position.xyz - world_pos;
  ...
  v_color = i_color;
  // Keep the standard engine varyings coherent (see the declarations): the
  // billboard's facing normal and the flipbook-cell UV the alpha mask samples.
  v_normal = to_eye;
  v_texture_coords = v_uv;
}
