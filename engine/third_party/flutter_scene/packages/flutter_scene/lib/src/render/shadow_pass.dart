...
///
/// [persistentColor] supplies the atlas texture instead of the transient
/// pool, so the atlas survives across frames. The pass overwrites the whole
/// atlas on every run (it is cleared first), so re-rendering into the same
/// texture is safe, and a frame whose inputs are unchanged can skip the pass
/// and keep sampling the previous content (see the scene's shadow retainer).
class ShadowPass extends RenderGraphPass {
  ...
  /// An atlas texture owned outside the transient pool (see the class doc).
  final gpu.Texture? persistentColor;
  ...
  void execute(RenderGraphContext context) {
    ...
    final requestedWidth = _tileResolution * totalTiles;
    ...
    // Single-channel r32Float: only the red channel is sampled, so the
    // three unused fp32 channels would otherwise cost 4x the bandwidth.
    final persistent = persistentColor;
    ...
    // The retained atlas grows only, so its width can exceed this frame's
    // tile strip; every attachment of one render target must still match.
    final atlasWidth = color.width;
