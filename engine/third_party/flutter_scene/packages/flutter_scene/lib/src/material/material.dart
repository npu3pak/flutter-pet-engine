...
abstract class Material {
  ...
  /// Translucent-pass draw priority: lower values draw earlier (farther in
  /// the back-to-front order), higher values later (on top of everything with
  /// a lower priority). 0 (the default) keeps the pure depth sort. Used by
  /// layered translucent effects — e.g. ground-mist clouds that must sit over
  /// the decor billboards they intersect, or a haze band that must stay
  /// behind them.
  double blendOrder = 0;
  ...
  ///
  /// Double-sided is honored for opaque and alpha-masked materials. A
  /// translucent ([AlphaMode.blend]) material is always back-face culled:
  /// drawing both sides would blend the overlapping front and back surfaces
  /// in triangle-index order rather than depth order (the translucent pass has
  /// no per-fragment sorting), which seams thick double-sided glass.
  @internal
