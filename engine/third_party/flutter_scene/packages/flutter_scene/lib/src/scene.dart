...
import 'render/shadow_atlas_retainer.dart';
...
base class Scene implements SceneGraph {
  ...
  // inputs (light, camera pose, caster set and transforms, spot shadows) are
  // bitwise identical to the last render skips the shadow pass entirely and
  // every view samples the previous content. Also lets several views of one
  // frame share the single pass. Created lazily on the first shadow frame.
  // See ShadowAtlasRetainer.
  ShadowAtlasRetainer? _shadowRetainer;
  ...
  void _renderViewToTexture({
    ...
    // the static shadow casters (geometry identity + full world transform),
    // so the shadow cache notices content appearing, vanishing, moving, or
    // rotating. It also records the exact shadow-caster content (visible
    // opaque items and their transforms) into the shadow retainer, so a
    // frame whose shadow inputs are unchanged can skip the shadow pass.
    final retainer = cascades.isNotEmpty || spotShadowFrame != null
        ? (_shadowRetainer ??= ShadowAtlasRetainer())
        : null;
    retainer?.beginProbe();
    ...
    for (final item in renderScene.items) {
      ...
      if (item.visible && item.material.isOpaque()) {
        // A visible opaque item can render into the shadow atlas this frame.
        retainer?.addCaster(item);
        if (item.shadowStatic) {
          hasStaticShadowCasters = true;
          final t = item.worldTransform.storage;
          var transformHash = 0;
          for (var k = 0; k < 16; k++) {
            transformHash =
                0x3fffffff & (transformHash * 31 + t[k].hashCode * (k + 1));
          }
    ...
    if (cascades.isNotEmpty || spotShadowFrame != null) {
      final tileResolution = cascades.isNotEmpty
          ? light!.shadowMapResolution
          : spotShadowFrame!.tileResolution;
      final casterFaces = cascades.isNotEmpty
          ? light!.shadowCasterFaces
          : ShadowCasterFaces.front;
      final totalTiles =
          cascades.length + (spotShadowFrame?.matrices.length ?? 0);
      ...
      final shadowUniform = cascades.isNotEmpty &&
          ...
          : null;
      final atlasRetainer = retainer!;
      // An amortized static-tile refresh (at most one cascade per frame) must
      // render even when the probe above matches: the refreshed tile's
      // content changes while the entry matrices and caster probes stay put.
      final forceShadowRender =
          shadowCachePlan?.refreshes.isNotEmpty ?? false;
      final contentMatches = !forceShadowRender &&
          atlasRetainer.contentMatches(
            cameraPosition: camera.position,
            tileResolution: tileResolution,
            totalTiles: totalTiles,
            casterFaces: casterFaces,
            spotFaces: spotShadowFrame?.casterFaces ?? ShadowCasterFaces.front,
            cascades: effectiveCascades,
            spotMatrices: spotShadowFrame?.matrices ?? const [],
          );
      // Reuse the retained atlas when it already holds bitwise-identical
      // content: within this frame (a previous view of the same camera just
      // rendered it) or across frames (nothing changed since the last
      // render). The pass is then skipped and its blackboard entries are
      // seeded from the retained texture.
      if (contentMatches) {
        graph.addPass(
          BlackboardSeedPass(<Object, Object?>{
            kShadowMapBlackboardKey: atlasRetainer.atlas,
            if (shadowUniform != null)
              kShadowUniformBlackboardKey: shadowUniform,
          }),
        );
      } else {
        final atlas = atlasRetainer.ensureAtlas(
          tileResolution * totalTiles,
          tileResolution,
        );
        graph.addPass(
          ShadowPass(
            ...
            persistentColor: atlas,
          ),
        );
        // The pass overwrites the whole atlas, so record the inputs it was
        // rendered with for the reuse decision of later views and frames.
        atlasRetainer.recordRendered();
      }
    }
