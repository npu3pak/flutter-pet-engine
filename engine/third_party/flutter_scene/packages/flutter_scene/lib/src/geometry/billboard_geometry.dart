...
enum BillboardFacing {
  ...
  /// Upright and uniformly screen-parallel: every quad shares one yaw
  /// ([BillboardGeometry.screenParallelYaw]) instead of per-instance camera
  /// facing — matches the decor billboards reoriented by a single yaw.
  screenParallel,
}
...
class BillboardGeometry extends Geometry {
  ...
  /// Yaw (radians) applied uniformly to every quad when [facing] is
  /// [BillboardFacing.screenParallel] — the same screen-parallel yaw used to
  /// reorient the decor billboard nodes.
  ...
  void bind(
    ...
    // [44] screen-parallel yaw
    frameInfo[44] = screenParallelYaw;
    pass.bindUniform(
  ...
  static double _facingValue(BillboardFacing facing) => switch (facing) {
    ...
    BillboardFacing.screenParallel => 3.0,
  };
