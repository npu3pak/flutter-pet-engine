...
/// camera-facing quad in the vertex shader and drawn in a single instanced
/// call. The ribbon has either a fixed world-space [width] or a constant
/// screen-pixel [pixelWidth] (so wireframes and gizmos keep the same visual
...
class LineSegmentsGeometry extends Geometry {
  ...
  ///
  /// [width] is the ribbon's world-space width. When [pixelWidth] is set, the
  /// ribbon keeps a constant screen-pixel width instead: the host must keep
  /// [pixelScale] in sync with the camera (see [pixelScaleFor]).
  /// [normalOffset] displaces endpoints along the source normals (ignored
  /// when the segment data carries none).
  ...
  /// The ribbon's screen-pixel width, or null for world-space [width] mode.
  /// Takes effect on the next frame.
  double? get pixelWidth => _pixelWidth;
  set pixelWidth(double? value) {
    _pixelWidth = value;
    _recomputeBounds();
  }

  /// The world size of one logical pixel at distance 1 for the active
  /// perspective camera (see [pixelScaleFor]); used only in [pixelWidth]
  /// mode. The host updates it when the camera or viewport changes.
  double pixelScale;

  /// The world-space size of one logical pixel at distance 1 for a
  /// perspective camera with vertical field of view [fovY] (radians) and a
  /// viewport [viewportHeight] logical pixels tall.
  static double pixelScaleFor({
    required double fovY,
    required double viewportHeight,
  }) =>
      viewportHeight > 0 ? 2 * math.tan(fovY / 2) / viewportHeight : 0.0;
  ...
  void bind(
    ...
    frameInfo[37] = _pixelWidth ?? 0.0;
    frameInfo[38] = pixelScale;
    // [39] unused
    pass.bindUniform(
