...
base class _TranslucentRecord {
  ...
  /// Translucent-pass layering from [Material.blendOrder] — priority groups
  /// sort before the depth (lower first, then far-to-near within a group).
...
base class SceneEncoder {
  ...
  void flushTranslucent({gpu.RenderPass? translucentPass}) {
    ...
    // Priority groups first (see [Material.blendOrder]), far-to-near within
    // each group — layered translucent effects stay deterministic regardless
    // of where two coplanar billboards happen to sit relative to the camera.
