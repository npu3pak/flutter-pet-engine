...
class GltfMaterial {
  ...
  /// Material authored with the `KHR_materials_pbrSpecularGlossiness`
  /// extension (fallback when [pbrMetallicRoughness] is absent).
  final GltfPbrSpecularGlossiness? pbrSpecularGlossiness;
...
/// The `KHR_materials_pbrSpecularGlossiness` extension: an alternative
/// PBR shading model authored as diffuse/specular/glossiness.
class GltfPbrSpecularGlossiness {
  GltfPbrSpecularGlossiness({
    this.diffuseFactor = const [1.0, 1.0, 1.0, 1.0],
    this.diffuseTexture,
    this.specularFactor = const [1.0, 1.0, 1.0],
    this.glossinessFactor = 1.0,
    this.specularGlossinessTexture,
  });

  final List<double> diffuseFactor;
  final GltfTextureInfo? diffuseTexture;
  final List<double> specularFactor;
  final double glossinessFactor;
  final GltfTextureInfo? specularGlossinessTexture;
}
