...
void main() {
  ...
  test('теневой атлас пересоздаётся при любом изменении раскладки', () {
    expect(
      ShadowAtlasRetainer.atlasNeedsResize(
        currentWidth: 0,
        currentHeight: 0,
        width: 4096,
        height: 1024,
    ...
    // Уменьшение числа каскадов тоже требует пересоздания: иначе UV шейдера
    // читают пустые тексели старого широкого атласа.
    expect(
      ShadowAtlasRetainer.atlasNeedsResize(
        currentWidth: 4096,
        currentHeight: 1024,
