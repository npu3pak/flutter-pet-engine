...
class SpriteAtlas {
  SpriteAtlas({required this.texture, required this.keys});
...
/// Composes [keys] into one row of `cellSize`² cells with a 1 px inset (so
/// bilinear sampling never bleeds between neighbours), using [load] for the
/// decoded images. Unresolvable keys are skipped. Returns null when nothing
/// resolves.
Future<({ui.Image image, List<String> keys})?> composeSpriteAtlas(
  List<String> keys,
  ...
  final image = await picture.toImage(images.length * cellSize, cellSize);
  picture.dispose();
...
/// (the texture upload touches the GPU).
Future<SpriteAtlas?> buildSpriteAtlas(
  ...
  try {
    ...
    return SpriteAtlas(texture: texture, keys: composed.keys);
  } finally {
